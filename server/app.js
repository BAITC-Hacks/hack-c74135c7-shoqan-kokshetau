...
import { authorize, checkVersion, fail, keys, newTask, object, proposalProgress, prototypeUrl, publicTask, publicProposal, rating, string, updateFields } from './domain.js';
...
const assets = new Map(['index.html', 'app.js', 'account.js', 'mentor.js', 'style.css'].map(name => [name, new URL('../public/' + name, import.meta.url)]));
...
export function createApp(store, { ai = clarify, mentor = mentorReply, translate = createTranslator(), origin = 'http://localhost:5173', authOptions, limitOptions } = {}) {
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if(req.method==='POST' && path==='/api/mentor')return send(200,await mentor(await readBody(req)));
      const viewTask = task => {
        let canManage = false;
      ...
      if (req.method === 'POST' && path === '/api/auth/reset-password') return send(200, await auth.resetPassword(req, res, await readBody(req)));
      ...
      if (req.method === 'GET' && path === '/api/proposals/mine') {
        if (!user) fail(401, 'Sign in required');
        const proposals = store.proposalsBySender(user.id).map(proposal => {
          const task = store.task(proposal.taskId);
          const available = task?.status === 'published';
          return { ...publicProposal(proposal), task: {
            id: proposal.taskId, available,
            // An unpublished revision is private. Show only the title seen at submission.
            title: available ? task.fields.title.value : proposal.taskTitleSnapshot || ''
          } };
        });
        return send(200, { proposals });
      }
      if (req.method === 'GET' && path === '/api/health') return send(200, { ok: true });
      if (req.method === 'GET' && path === '/api/tasks') return send(200, { tasks: store.tasks().filter(t => t.status === 'published').sort((a,b)=>rating(b.fields).score-rating(a.fields).score || b.createdAt.localeCompare(a.createdAt)).map(viewTask) });
      if (req.method === 'POST' && path === '/api/tasks/draft') {
        const body = await readBody(req); keys(body, ['description','topic']);
        const { task, ownerToken } = newTask(body.description);
        if(body.topic!==undefined)task.topic=string(body.topic,100);
      ...
      if (match) {
        ...
        if (req.method === 'PATCH' && !action) {
          const body = await readBody(req); keys(body, ['version', 'fields','topic']);
          const task = store.transaction(() => {
            const current = ownedTask(id); checkVersion(current, body.version);
            const updated = updateFields(current, body.fields);
            if(body.topic!==undefined)updated.topic=string(body.topic,100);
            store.saveTask(updated); return updated;
          });
          return send(200, { task: viewTask(task) });
        }
        ...
        if (req.method === 'GET' && action === 'proposals') {
          ownedTask(id); return send(200, { proposals: store.proposals(id).map(publicProposal) });
        }
        if (req.method === 'POST' && action === 'proposals') {
          const body = await readBody(req); keys(body, ['teamName', 'contact', 'message', 'plan', 'deadline', 'prototypeLink', 'skills', 'interests', 'technologies']);
          const proposal = store.transaction(() => {
            ...
            const now = new Date().toISOString();
            const result = {
              id: randomUUID(), taskId: id, teamName: string(body.teamName, 200), contact: string(body.contact, 1000),
              message: string(body.message), plan: string(body.plan), deadline: string(body.deadline, 200),
              prototypeLink: prototypeUrl(body.prototypeLink), skills: string(body.skills ?? '', 2000, true),
              interests: string(body.interests ?? '', 2000, true), technologies: string(body.technologies ?? '', 2000, true),
              progress: 0, progressPoints: 0, status: 'pending', createdAt: now, decidedAt: null, version: 1
            };
            if (user) {
              result.senderId = user.id;
              result.taskTitleSnapshot = task.fields.title.value;
            }
            store.saveProposal(result); return result;
          });
          return send(201, { proposal: publicProposal(proposal), tracked: Boolean(proposal.senderId) });
        }
      }
      const proposalMatch = path.match(/^\/api\/proposals\/([^/]+)$/);
      if (req.method === 'PATCH' && proposalMatch) {
        const body = await readBody(req); keys(body, ['status', 'version', 'decisionNote', 'progress']);
        const proposal = store.transaction(() => {
          ...
          const decisionNote = body.decisionNote === undefined ? (body.progress === undefined ? '' : current.decisionNote || '') : string(body.decisionNote, 2000, true);
          if (body.progress !== undefined && (body.status !== 'accepted' || current.status !== 'accepted')) fail(409, 'Accept the proposal before confirming progress');
          const progress = body.status==='rejected' ? proposalProgress(0) : body.progress === undefined ? proposalProgress(current.progress || 0) : proposalProgress(body.progress);
          const updated = { ...current, status: body.status, decisionNote, ...progress, decidedAt: new Date().toISOString(), version: current.version + 1 };
