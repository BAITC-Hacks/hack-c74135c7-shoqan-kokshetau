import { accountMessages, authMarkup, resetMarkup } from './account.js';
import { showMentor } from './mentor.js';

const dictionary = {
  kk: {
    mentor:'AI тәлімгер',
    topicExample:'Мысалы: білім беру, сауда, туризм', improveRating:'Ұпайды арттыру үшін толтырып, растаңыз',
    publishedEditWarning:'Бұл тапсырма жарияланған. Өзгерістерді сақтағанда ол каталогтан уақытша алынады. Тексеріп, қайта жариялаңыз. Бұрынғы ұсыныстар сақталады.',
    titleRequired:'Тапсырма атауын жазыңыз', confirmFields:'Мына өрістерді растаңыз:', fieldsReady:'Өрістер дайын. Карточканы қарап, төмендегі растау белгісін қойыңыз.', emptyConfirm:'Растау үшін алдымен осы өрісті толтырыңыз.',
  ...
  ru: {
    mentor:'AI наставник',
    topicExample:'Например: образование, торговля, туризм', improveRating:'Заполните и подтвердите, чтобы повысить оценку',
    publishedEditWarning:'Эта задача опубликована. После сохранения изменений она временно исчезнет из каталога. Проверьте и опубликуйте её повторно. Прежние предложения сохранятся.',
    titleRequired:'Введите название задачи', confirmFields:'Подтвердите следующие поля:', fieldsReady:'Поля готовы. Проверьте карточку и поставьте отметку ниже.', emptyConfirm:'Сначала заполните это поле, чтобы подтвердить его.',
...
Object.assign(dictionary.kk, { allTopics:'Тақырып', allTopicsOption:'Барлық тақырып', levelFilter:'Деңгей', plan:'Іске асыру жоспары', deadline:'Мерзім', prototypeLink:'Прототип сілтемесі', skills:'Дағдылар', interests:'Қызығушылықтар', technologies:'Технологиялар', progress:'Нақты прогресс', progressHelp:'Тек қабылданған команда үшін кезеңді растаңыз.', progress0:'Басталмады', progress1:'Идея расталды', progress2:'Прототип дайын', progress3:'Нәтиже тапсырылды', progressSaved:'Прогресс сақталды' });
Object.assign(dictionary.ru, { allTopics:'Тема', allTopicsOption:'Все темы', levelFilter:'Уровень', plan:'План реализации', deadline:'Срок', prototypeLink:'Ссылка на прототип', skills:'Навыки', interests:'Интересы', technologies:'Технологии', progress:'Фактический прогресс', progressHelp:'Подтверждайте этап только для принятой команды.', progress0:'Не начат', progress1:'Идея подтверждена', progress2:'Прототип готов', progress3:'Результат передан', progressSaved:'Прогресс сохранён' });
...
let tasks = [], current = null, proposals = [], questions = null, translations = new Map(), translation = null, user = null, sentProposals = [], proposalFilter = 'all';
let decisionNotes = Object.create(null);
let routeError = null;
let authReturn = 'mine';
function isAuthPage(value) { return value==='login' || value==='register' || value==='forgot'; }
function authDestination(value) {
  return /^(?:catalog|mine|outbox|new|(?:task|edit|proposals)\/[^/?#]+)$/.test(value) ? value : 'mine';
}
let page = '', seq = 0, dirty = false, showOriginal = false, busy = false, loading = false, query = '', sort = 'score', topicFilter = 'all', levelFilter = 'all', toastTimer;
...
async function api(path, method = 'GET', body, owner) {
  ...
  catch { throw Object.assign(new Error(t('networkError')), {messageKey:'networkError'}); }
  const result = await response.json();
  if (!response.ok) {
    const authRequest=path.startsWith('/auth/');
    if(path==='/auth/reset-password' && response.status===401)throw new Error(t('resetInvalid'));
    ...
    throw Object.assign(new Error(t(errorKey || 'error')), {status:response.status,messageKey:errorKey || 'error'});
  }
  return result;
}
...
function accountChrome() {
  ...
  nav.lastElementChild.insertAdjacentHTML('beforebegin',`<a class="nav-link ${page==='mentor'?'active':''}" href="#mentor" ${page==='mentor'?'aria-current="page"':''}>${icon('spark')}${t('mentor')}</a>`);
  nav.lastElementChild.insertAdjacentHTML('beforebegin',`<a class="nav-link ${page==='outbox'?'active':''}" href="#outbox" ${page==='outbox'?'aria-current="page"':''}>${icon('chat')}${t('outbox')}</a>`);
  ...
  if(page==='login' || page==='register' || page==='outbox' || page==='mentor'){
    document.querySelector('.breadcrumb strong').textContent=t(page);
    document.querySelectorAll('.nav-link.active').forEach(link=>{if(link.getAttribute('href')!==`#${page}`){link.classList.remove('active');link.removeAttribute('aria-current');}});
  }
}
...
function resetPage(snapshot) {
  shell(resetMarkup(t,esc,snapshot));
  document.title=`Shoqan — ${t('resetTitle')}`;
  document.querySelector('.breadcrumb strong').textContent=t('resetTitle');
}
...
function ensureCatalogFilters() {
  ...
  if (!toolbar || document.querySelector('#topic-filter')) return;
  const topics = [...new Set(tasks.map(task => task.topic || 'Другое'))].sort();
  toolbar.insertAdjacentHTML('beforeend', `<select id="topic-filter" aria-label="${t('allTopics')}"><option value="all" ${topicFilter==='all'?'selected':''}>${t('allTopicsOption')}</option>${topics.map(topic=>`<option value="${esc(topic)}" ${topicFilter===topic?'selected':''}>${esc(topic)}</option>`).join('')}</select><select id="level-filter" aria-label="${t('levelFilter')}"><option value="all" ${levelFilter==='all'?'selected':''}>${t('levelFilter')}</option><option value="priority" ${levelFilter==='priority'?'selected':''}>${t('level3')}</option><option value="ready" ${levelFilter==='ready'?'selected':''}>${t('level2')}</option><option value="working" ${levelFilter==='working'?'selected':''}>${t('level1')}</option><option value="draft" ${levelFilter==='draft'?'selected':''}>${t('level0')}</option></select>`);
}
function renderCards(mine = false) {
  ...
  const list = tasks.filter(task => {
    ...
    const score = task.rating.score;
    const taskLevel = score < 40 ? 'draft' : score < 70 ? 'working' : score < 90 ? 'ready' : 'priority';
    return `${task.fields.title.value} ${task.fields.context.value} ${tr?.title || ''} ${tr?.context || ''}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())
      && (topicFilter === 'all' || (task.topic || 'Другое') === topicFilter)
      && (levelFilter === 'all' || taskLevel === levelFilter);
...
function topicInput(value = '') {
  return `<div class="form-group"><label class="form-label" for="task-topic">${t('allTopics')}</label><input id="task-topic" name="topic" required maxlength="100" value="${esc(value)}" placeholder="${t('topicExample')}"></div>`;
}
function newPage(description = '') {
  ...
  document.querySelector('#new-form').insertAdjacentHTML('afterbegin',topicInput());
...
function editor(snapshot, topicSnapshot) {
  ...
  document.querySelector('#edit-form').insertAdjacentHTML('afterbegin',topicInput(topicSnapshot ?? current.topic));
  document.querySelector('.publish-box > p').insertAdjacentHTML('afterend','<div id="publish-readiness" role="status" aria-live="polite"></div>');
  if(current.status==='published')document.querySelector('#edit-form').insertAdjacentHTML('beforebegin',`<div class="notice warning">${t('publishedEditWarning')}</div>`);
...
function publicationIssues(values) {
  return {
    missingTitle: !values.title.value.trim(),
    unconfirmed: fields.filter(name=>values[name].value.trim() && !values[name].confirmed)
  };
}
function updatePublicationReadiness() {
  const node=document.querySelector('#publish-readiness');
  if(!node)return;
  const values=formValues();
  for(const name of fields){
    const checkbox=document.querySelector(`[data-confirm="${name}"]`);
    checkbox.disabled=!values[name].value.trim();
    checkbox.title=checkbox.disabled?t('emptyConfirm'):'';
  }
  const {missingTitle,unconfirmed}=publicationIssues(values);
  const improvements=fields.filter(name=>name!=='title' && (!values[name].value.trim() || !values[name].confirmed));
  node.innerHTML=`${missingTitle?`<p><button type="button" class="btn small" data-focus-field="title">${t('titleRequired')}</button></p>`:''}${unconfirmed.length?`<p>${t('confirmFields')}</p><div class="actions">${unconfirmed.map(name=>`<button type="button" class="btn small" data-focus-field="${name}">${t(name)}</button>`).join('')}</div>`:''}<p>${dirty?t('saveFirst'):!missingTitle&&!unconfirmed.length?t('fieldsReady'):''}</p>`;
  if(improvements.length)node.insertAdjacentHTML('beforeend',`<p>${t('improveRating')}</p><ul>${improvements.map(name=>`<li>${t(name)}: +${current.rating.breakdown.find(item=>item.field===name)?.max || 0}</li>`).join('')}</ul>`);
}
...
function enhanceProposalForm(form) {
  if (!form || form.querySelector('[name="plan"]')) return;
  form.querySelector('[name="message"]')?.closest('.form-group').insertAdjacentHTML('afterend', `<div class="form-group"><label class="form-label" for="proposal-plan">${t('plan')}</label><textarea id="proposal-plan" name="plan" required maxlength="10000"></textarea></div><div class="form-group"><label class="form-label" for="proposal-deadline">${t('deadline')}</label><input id="proposal-deadline" name="deadline" required maxlength="200"></div><div class="form-group"><label class="form-label" for="proposal-prototypeLink">${t('prototypeLink')}</label><input id="proposal-prototypeLink" name="prototypeLink" type="url" required maxlength="2000"></div><div class="form-group"><label class="form-label" for="proposal-skills">${t('skills')}</label><input id="proposal-skills" name="skills" maxlength="2000"></div><div class="form-group"><label class="form-label" for="proposal-interests">${t('interests')}</label><input id="proposal-interests" name="interests" maxlength="2000"></div><div class="form-group"><label class="form-label" for="proposal-technologies">${t('technologies')}</label><input id="proposal-technologies" name="technologies" maxlength="2000"></div>`);
}
function detail() {
  ...
  if(form)form.insertAdjacentHTML('beforebegin',`<div class="notice">${t(user?'proposalTracking':'proposalGuest')}${user?'':` <a class="text-link" href="#login">${t('login')} →</a>`}</div>`);
...
function enhanceProposalCards() {
  document.querySelectorAll('.proposal').forEach((card, index) => {
    const proposal = proposals[index];
    if (!proposal || card.querySelector('[data-progress]')) return;
    card.querySelector('.proposal-head').insertAdjacentHTML('afterend', `<dl class="proposal-details"><div><dt>${t('plan')}</dt><dd>${esc(proposal.plan || t('noValue'))}</dd></div><div><dt>${t('deadline')}</dt><dd>${esc(proposal.deadline || t('noValue'))}</dd></div><div><dt>${t('prototypeLink')}</dt><dd>${safePrototypeUrl(proposal.prototypeLink) ? `<a href="${esc(safePrototypeUrl(proposal.prototypeLink))}" rel="noreferrer">${esc(proposal.prototypeLink)}</a>` : t('noValue')}</dd></div><div><dt>${t('interests')}</dt><dd>${esc(proposal.interests || t('noValue'))}</dd></div><div><dt>${t('skills')}</dt><dd>${esc(proposal.skills || t('noValue'))}</dd></div><div><dt>${t('technologies')}</dt><dd>${esc(proposal.technologies || t('noValue'))}</dd></div></dl><div class="progress-control"><strong>${t('progress')}: ${proposal.progressPoints || 0} ${t('points')}</strong><small>${t('progressHelp')}</small><div class="actions">${[0,1,2,3].map(stage=>`<button class="btn small" data-progress="${stage}" data-id="${esc(proposal.id)}" ${proposal.status!=='accepted' || proposal.progress===stage?'disabled':''}>${t('progress'+stage)} (${stage*10})</button>`).join('')}</div></div>`);
  });
}
function proposalPage() {
  const cards = proposals.map(proposal => {
    const changed = Object.hasOwn(decisionNotes, proposal.id);
    const note = changed ? decisionNotes[proposal.id] : proposal.decisionNote || '';
    return `<article class="proposal">
      <div class="proposal-head"><h3>${esc(proposal.teamName)}</h3><span class="badge ${proposal.status==='accepted'?'good':proposal.status==='pending'?'warm':''}">${t(proposal.status)}</span></div>
      <p>${esc(proposal.message)}</p><p><strong>${t('teamContact')}:</strong> ${esc(proposal.contact)}</p>
      <div class="form-group"><label class="form-label" for="note-${esc(proposal.id)}">${t('decisionNote')}</label>
        <textarea id="note-${esc(proposal.id)}" data-decision-note="${esc(proposal.id)}" maxlength="2000" placeholder="${t('decisionNotePlaceholder')}" aria-describedby="note-help-${esc(proposal.id)}">${esc(note)}</textarea>
        <small id="note-help-${esc(proposal.id)}">${t('decisionNoteHelp')}</small>
      </div>
      <div class="actions"><button class="btn soft" data-decision="accepted" data-id="${esc(proposal.id)}" ${proposal.status==='accepted'&&!changed?'disabled':''}>${icon('check')}${t('accept')}</button>
      <button class="btn danger" data-decision="rejected" data-id="${esc(proposal.id)}" ${proposal.status==='rejected'&&!changed?'disabled':''}>${t('reject')}</button></div>
    </article>`;
  }).join('');
  shell(`<a class="back" href="#edit/${esc(current.id)}">${icon('back')}${t('edit')}</a><div class="page-heading"><div><h1>${t('proposals')}</h1><p class="subtitle">${esc(current.fields.title.value || current.description)}</p></div><span class="count">${proposals.length}</span></div><section class="panel">${proposals.length?`<div class="notice">${t('feedbackGuestHelp')}</div>${cards}`:empty('noProposals','noProposalsText')}</section>`);
  enhanceProposalCards();
}
function outboxPage() {
  const date=value=>new Intl.DateTimeFormat(lang==='kk'?'kk-KZ':'ru-RU',{dateStyle:'medium',timeStyle:'short'}).format(new Date(value));
  const visible=sentProposals.filter(proposal=>proposalFilter==='all'||proposal.status===proposalFilter);
  shell(`<div class="page-heading"><div><h1>${t('outbox')}</h1><p class="subtitle">${t('outboxHelp')}</p></div>${user?`<button class="btn" data-action="refresh-outbox">${t('refresh')}</button>`:''}</div>${!user ? empty('outboxSignIn','outboxSignInHelp',`<a class="btn primary" href="#login">${t('login')}</a>`) : `<div class="toolbar"><select id="proposal-filter" aria-label="${t('allStatuses')}">${['all','pending','accepted','rejected'].map(status=>`<option value="${status}" ${proposalFilter===status?'selected':''}>${t(status==='all'?'allStatuses':status)}</option>`).join('')}</select><a class="btn soft" href="#catalog">${t('browseTasks')} →</a></div><div class="outbox-list">${visible.length ? visible.map(proposal=>`<article class="panel outbox-card"><div class="proposal-head"><div><div class="eyebrow">${esc(proposal.teamName)}</div><h2>${esc(proposal.task.title || t('task'))}</h2></div><span class="badge ${proposal.status==='accepted'?'good':proposal.status==='pending'?'warm':''}">${t(proposal.status)}</span></div><p class="proposal-message">${esc(proposal.message)}</p><p class="outbox-contact"><strong>${t('teamContact')}:</strong> ${esc(proposal.contact)}</p><div class="outbox-meta"><span>${t('sentAt')}: ${esc(date(proposal.createdAt))}</span>${proposal.decidedAt?`<span>${t('decidedAt')}: ${esc(date(proposal.decidedAt))}</span>`:''}</div>${proposal.task.available?`<a class="btn small" href="#task/${esc(proposal.taskId)}">${t('details')} →</a>`:`<div class="notice warning">${t('taskUnavailable')}</div>`}</article>`).join(''):empty(sentProposals.length?'outboxFilterEmpty':'outboxEmpty',sentProposals.length?'outboxFilterHelp':'outboxEmptyHelp')}</div>`}`);
  document.title=`Shoqan — ${t('outbox')}`;
  document.querySelectorAll('.outbox-card').forEach((card,index)=>{
    const item=visible[index];
    card.querySelector('.proposal-message').insertAdjacentHTML('afterend',`<p><strong>${t('progress')}:</strong> ${t('progress'+(item.progress || 0))} · ${item.progressPoints || 0} ${t('points')}</p><p><strong>${t('plan')}:</strong> ${esc(item.plan || t('noValue'))}</p><p><strong>${t('deadline')}:</strong> ${esc(item.deadline || t('noValue'))}</p>`);
    const note=visible[index].decisionNote;
    if(note)card.querySelector('.proposal-message').insertAdjacentHTML('afterend',`<section class="decision-feedback"><h3>${t('businessFeedback')}</h3><p>${esc(note)}</p></section>`);
  });
}
...
function errorPage() {
  shell(`<div class="panel"><h2>${t('error')}</h2><p>${esc(routeError.messageKey ? t(routeError.messageKey) : routeError.message)}</p><button class="btn" data-action="reload">${t('reload')}</button> <a class="btn" href="#catalog">${t('back')}</a></div>`);
}
async function route() {
  const generation = ++seq;
  loading=true;
  const nextPage=location.hash.slice(1) || 'catalog';
  if(isAuthPage(nextPage) && !isAuthPage(page))authReturn=authDestination(page);
  page = nextPage; dirty=false; questions=null; translation=null; showOriginal=false;
  decisionNotes=Object.create(null);
  routeError=null; current=null; proposals=[]; tasks=[]; sentProposals=[];
  ...
  try {
    ...
    if(page==='forgot'){resetPage();return;}
    if(page==='mentor'){showMentor({shell,user,lang,act});return;}
    if(page==='outbox'){
      const results=user ? (await api('/proposals/mine')).proposals : [];
      if(seq!==generation)return;
      sentProposals=results;
      outboxPage();return;
    }
    ...
    if(seq!==generation)return;
    current=result.task;
    if(view!=='task' && !canManage(current)) throw Object.assign(new Error(t('forbidden')), {messageKey:'forbidden'});
  ...
  } catch(error) { if(seq===generation){routeError=error;current=null;proposals=[];errorPage();} }
...
async function switchLanguage(language) {
  ...
  const snapshot=document.querySelector('#edit-form') ? formValues():null;
  const description=document.querySelector('#description')?.value;
  const topicValue=document.querySelector('#task-topic')?.value;
  ...
  const resetValues=resetForm ? Object.fromEntries(new FormData(resetForm)):null;
  const wasDirty=dirty;
  lang=language; try{localStorage.setItem('shoqan-language',JSON.stringify(lang));}catch{}
  ++seq;
  if(routeError)errorPage();
  else if(page==='login' || page==='register'){
    authPage(page==='register',authValues);
    if(authValues)document.querySelector('#auth-password').value=authValues.password;
  }
  else if(page==='forgot'){
    resetPage(resetValues || undefined);
    if(resetValues){document.querySelector('#reset-password').value=resetValues.password;document.querySelector('#reset-confirm').value=resetValues.confirm;}
  }
  else if(page==='mentor')showMentor({shell,user,lang,act});
  else if(page==='outbox')outboxPage();
  else if(page==='new'){newPage(description);document.querySelector('#task-topic').value=topicValue || '';}
  else if(page.startsWith('edit/') && current){questions=null;editor(snapshot,topicValue);}
...
app.addEventListener('input',event=>{
  if(event.target.dataset.decisionNote){
    const id=event.target.dataset.decisionNote;
    const proposal=proposals.find(item=>item.id===id);
    if(event.target.value===(proposal.decisionNote||''))delete decisionNotes[id];
    else decisionNotes[id]=event.target.value;
    dirty=Object.keys(decisionNotes).length>0;
    event.target.closest('.proposal').querySelectorAll('[data-decision]').forEach(button=>{
      button.disabled=button.dataset.decision===proposal.status&&!Object.hasOwn(decisionNotes,id);
    });
    return;
  }
  ...
  if(event.target.closest('#new-form') || event.target.closest('#proposal-form') || event.target.closest('#reset-form'))dirty=true;
});
app.addEventListener('change',event=>{
  if(event.target.id==='sort'){sort=event.target.value;renderCards(page==='mine');}
  if(event.target.id==='topic-filter'){topicFilter=event.target.value;renderCards(page==='mine');}
  if(event.target.id==='level-filter'){levelFilter=event.target.value;renderCards(page==='mine');}
  if(event.target.id==='proposal-filter'){proposalFilter=event.target.value;outboxPage();}
});
app.addEventListener('click',event=>{
  const link=event.target.closest('a[href^="#"]');
  if(link){
    if(link.getAttribute('href')==='#main'){event.preventDefault();document.querySelector('#main')?.focus();return;}
    if(link.getAttribute('href')===`#${page}`){event.preventDefault();return;}
  ...
  if(button.dataset.focusField){document.querySelector(`#field-${button.dataset.focusField}`)?.focus();return;}
  if(button.dataset.progress)void act(button,async()=>{
    if(dirty)throw new Error(t('saveFirst'));
    const proposal=proposals.find(item=>item.id===button.dataset.id);
    const result=await api(`/proposals/${encodeURIComponent(proposal.id)}`,'PATCH',{version:proposal.version,status:proposal.status,progress:Number(button.dataset.progress)},token(current.id));
    proposals=proposals.map(item=>item.id===proposal.id?result.proposal:item);
    proposalPage();toast(t('progressSaved'));
  });
  if(button.dataset.decision)void act(button,async()=>{
    const proposal=proposals.find(p=>p.id===button.dataset.id);
    const decisionNote=decisionNotes[proposal.id] ?? proposal.decisionNote ?? '';
    const result=await api(`/proposals/${encodeURIComponent(proposal.id)}`,'PATCH',{version:proposal.version,status:button.dataset.decision,decisionNote},token(current.id));
    proposals=proposals.map(p=>p.id===proposal.id?result.proposal:p);
    delete decisionNotes[proposal.id];dirty=Object.keys(decisionNotes).length>0;
    proposalPage();toast(t('decisionSaved'));
  });
  const action=button.dataset.action;
  if(action==='refresh-outbox')void act(button,async()=>{await route();});
  if(action==='logout'){
    if(dirty && !confirm(t('unsaved')))return;
    void act(button,async()=>{
      await api('/auth/logout','POST',{});user=null;dirty=false;current=null;proposals=[];sentProposals=[];translations.clear();
  ...
  if(action==='publish')void act(button,async()=>{
    if(dirty)throw new Error(t('saveFirst'));
    const issues=publicationIssues(current.fields);
    if(issues.missingTitle || issues.unconfirmed.length){
      throw new Error(issues.missingTitle?t('titleRequired'):`${t('confirmFields')} ${issues.unconfirmed.map(t).join(', ')}`);
    }
...
app.addEventListener('submit',event=>{
  ...
  void act(button,async()=>{
    if(form.id==='reset-form'){
      ...
      if(values.password!==values.confirm)throw new Error(t('passwordMismatch'));
      await api('/auth/reset-password','POST',{token:values.token,password:values.password});
      form.reset();user=null;dirty=false;current=null;proposals=[];sentProposals=[];translations.clear();
      toast(t('resetSuccess'));location.hash='login';
    }
    if(form.id==='auth-form'){
      try{
        const result=await api(`/auth/${form.dataset.mode}`,'POST',Object.fromEntries(new FormData(form)));
        form.reset();user=result.user;dirty=false;translations.clear();toast(t('authSuccess'));location.hash=authReturn;
      }catch(error){
    ...
    if(form.id==='new-form'){
      ...
      const result=await api('/tasks/draft','POST',{description,topic:document.querySelector('#task-topic').value.trim()});if(result.ownerToken)storeOwner(result.task.id,result.ownerToken);dirty=false;location.hash=`edit/${result.task.id}`;
    }
    if(form.id==='edit-form'){
      const result=await api(`/tasks/${encodeURIComponent(current.id)}`,'PATCH',{version:current.version,fields:formValues(),topic:document.querySelector('#task-topic').value.trim()},token(current.id));
      current=result.task;dirty=false;editor();toast(t('saved'));
    }
    if(form.id==='proposal-form'){
      const values=Object.fromEntries(new FormData(form));if(['teamName','contact','message','plan','deadline','prototypeLink'].some(name=>!values[name]?.trim()))throw new Error(t('invalid'));
      const result=await api(`/tasks/${encodeURIComponent(current.id)}/proposals`,'POST',values);form.reset();dirty=false;toast(t(result.tracked?'proposalSaved':'proposalSent'));
    }
